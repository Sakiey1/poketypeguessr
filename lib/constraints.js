 ...
 *
 * The caller is responsible for picking the combo. We only return a constraint
 * if at least MIN_POOL_SIZE Pokemon in `pool` satisfy combo+constraint.
 */
const generateConstraint = ({
  ...
  for (const category of shuffledCategories) {
    ...
    for (const candidate of candidates) {
      if (countMatching(pool, combo, candidate.predicates) >= MIN_POOL_SIZE) {
        return candidate;
      }
    }
  }
